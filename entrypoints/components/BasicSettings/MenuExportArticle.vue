...
export default {
  ...
  methods: {
    ...
    showExportDialog() {
      ...
      const pageTitle = document.title.replace(' - LinuxDo', '').trim() || 'discourse_export';
      
      // 检测是否为暗黑模式
      const isDarkMode = $('html').hasClass('discourse-dark-mode') || $('html').hasClass('dark-mode') || $('html').hasClass('night-mode');
      ...
      // 创建导出对话框
      const dialogHTML = `
        <div class="linuxdoscripts-export-dialog" style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:${styles.background};color:${styles.text};border:1px solid ${styles.border};border-radius:8px;padding:24px;z-index:9999;box-shadow:0 0 20px rgba(0,0,0,0.3);min-width:350px;font-family:system-ui,-apple-system,sans-serif;">
          <h3 style="margin-top:0;margin-bottom:16px;border-bottom:1px solid ${styles.divider};padding-bottom:12px;font-size:18px;font-weight:500;">导出设置</h3>
          <div style="margin:15px 0;">
            <label style="display:flex;align-items:center;margin-bottom:16px;cursor:pointer;">
              <input type="checkbox" id="export-with-comments" checked style="margin-right:10px;width:16px;height:16px;"> 
              <span>包含评论</span>
            </label>
            <div id="comments-level-container" style="margin-top:16px;background:${styles.inputBg};border-radius:6px;padding:12px;border:1px solid ${styles.border};">
              <label style="display:block;margin-bottom:10px;font-weight:500;">导出楼层数: <span id="selected-level" style="color:${styles.primary};">${commentCount}</span></label>
              <div style="display:flex;align-items:center;margin-top:10px;">
                <input type="range" id="comments-level" min="0" max="${commentCount}" value="${commentCount}" style="flex:1;margin-right:10px;accent-color:${styles.primary};">
              </div>
              <div style="font-size:12px;color:${styles.hint};margin-top:8px;display:flex;justify-content:space-between;">
                <span>仅主楼</span>
                <span>全部 (${commentCount}楼)</span>
              </div>
              <div style="font-size:12px;color:${isDarkMode ? '#ff9d9d' : '#d94f4f'};margin-top:10px;padding-top:8px;border-top:1px dashed ${styles.border};">
          ...
          <div style="text-align:right;margin-top:20px;border-top:1px solid ${styles.divider};padding-top:16px;display:flex;justify-content:flex-end;gap:12px;">
            <button id="cancel-export" style="padding:8px 16px;background:${styles.buttonBg};color:${styles.buttonText};border:1px solid ${styles.border};border-radius:4px;cursor:pointer;font-size:14px;transition:all 0.2s;">取消</button>
            <button id="confirm-export" style="padding:8px 16px;background:${styles.primary};color:white;border:none;border-radius:4px;cursor:pointer;font-size:14px;transition:all 0.2s;">确认导出</button>
          </div>
        </div>
        <div class="linuxdoscripts-export-overlay" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,${isDarkMode ? '0.7' : '0.5'});z-index:9998;"></div>
      `;
      ...
      $('#cancel-export').on('click', () => {
        $('.linuxdoscripts-export-dialog, .linuxdoscripts-export-overlay, .linuxdoscripts-export-style').remove();
      });
    ...
    exportMarkdown(includeComments = true, commentLevel = 0, fileName = 'discourse_export') {
